...
export function EditorClient({ initialProjects, activeProjectId }: EditorClientProps) {
  const router = useRouter();
  const [activeNode, setActiveNode] = useState<string | null>(null);

  // Dedicated project actions hook
  const {
    activeDialog,
    selectedProject,
    name,
  ...
  // Clear active node selection if the active project changes
  useEffect(() => {
    setActiveNode(null);
  }, [activeProjectId]);

  const activeProject = initialProjects.find((p) => p.id === activeProjectId);
